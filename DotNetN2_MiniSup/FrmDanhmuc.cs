...
namespace DotNetN2_MiniSup
{
    public partial class FrmDanhmuc : Form
    {
        ...
        private void btn_timkiem_Click(object sender, EventArgs e)
        {
            ...
            {
                if(cb_ma.Checked==false && cb_ten.Checked==false) MessageBox.Show("Vui chọn loại danh mục bạn muốn tìm kiếm !!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                else
                {
                    if(cb_ma.Checked )
                    {
        ...
        {
            if(e .RowIndex > 0) {
                txt_madm.Text = dataGridView1.Rows[e.RowIndex].Cells["madm"].Value.ToString();
                txt_tendm.Text = dataGridView1.Rows[e.RowIndex].Cells["tendm"].Value.ToString();
            }
        }
    }
}
