...
namespace DotNetN2_MiniSup
{
    public partial class Frmnhacc : Form
    {
        ...
        {
            string tennhacc = txt_tenncc.Text;
            if (string.IsNullOrWhiteSpace(tennhacc)) {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > 0)
            {
                txt_mancc.Text = dataGridView1.Rows[e.RowIndex].Cells["manhacc"].Value.ToString();
                txt_tenncc.Text = dataGridView1.Rows[e.RowIndex].Cells["tennhacc"].Value.ToString();
