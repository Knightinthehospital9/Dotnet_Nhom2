...
namespace DotNetN2_MiniSup
{
    public partial class FrmKhachhang : Form
    {
        ...
        private void btn_timkiem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Vui nhập mã khách hàng muốn tìm kiếm !!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                dataGridView1.DataSource = null;
                dataGridView1.Rows.Clear();
                dataGridView1.DataSource = khachhang.SearchKH(timkiem);
            }
        }
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > 0)
            {
                txt_makh.Text = dataGridView1.Rows[e.RowIndex].Cells["makh"].Value.ToString();
                txt_tenkh.Text = dataGridView1.Rows[e.RowIndex].Cells["tenkh"].Value.ToString();
